...
public class STANGA_5H_PLUS_MARKER extends LinearOpMode {
    enum STROBOT
    {
        START,
        PLACE_PRELOAD,
    ...
    public void runOpMode() throws InterruptedException {
        ...
        AtomicReference<STANGA_5H_PLUS.STROBOT> status = new AtomicReference<>(STANGA_5H_PLUS.STROBOT.START);
        ...
        TrajectorySequence PLACE_PRELOAD = drive.trajectorySequenceBuilder(startPose)
                ...
                .back(10)
                .splineTo(new Vector2d(-15, -35), Math.toRadians(40))
                .back(10)
...
//                .setConstraints(drive.getVelocityConstraint(35, 10, 11.19), drive.getAccelerationConstraint(35))
                ...
                .splineTo(new Vector2d(-6, -19), Math.toRadians(320))
...
//                .setConstraints(drive.getVelocityConstraint(35, 10, 11.19), drive.getAccelerationConstraint(35))
                ...
                .splineTo(new Vector2d(-40, -14.5), Math.toRadians(180))
                .splineTo(new Vector2d(-66, -12), Math.toRadians(180))
                .addSpatialMarker(new Vector2d(-15, -12), () -> {
                    junctionHeight = 7;
                    liftController.CurrentStatus = LiftController.liftStatus.POLE;
...
//                .waitSeconds(1)
                ...
                .forward(1)
                .splineTo(new Vector2d(-35, -15), Math.toRadians(180))
...
//                .waitSeconds(1)
        ...
        while (opModeIsActive() && !isStopRequested())
        {
            ...
            if (status.get() == STANGA_5H_PLUS.STROBOT.START && !drive.isBusy() && trajCount == 0)
            {
                trajCount++;
                drive.followTrajectorySequenceAsync(PLACE_PRELOAD);
                status.set(STANGA_5H_PLUS.STROBOT.PLACE_PRELOAD);
            }

            if(((status.get() == STANGA_5H_PLUS.STROBOT.PLACE_PRELOAD) || (status.get() == STANGA_5H_PLUS.STROBOT.GO_PLACE_FROM_STACK)) && !drive.isBusy() && trajCount == 1) {
                trajCount++;
                drive.followTrajectorySequenceAsync(GTS_FIRST);
                status.set(STANGA_5H_PLUS.STROBOT.COLLECT);
            }

            if (status.get() == STANGA_5H_PLUS.STROBOT.COLLECT && !drive.isBusy() && trajCount == 2)
            {
                trajCount++;
                drive.followTrajectorySequenceAsync(PLACE_FIRST);
                status.set(STANGA_5H_PLUS.STROBOT.GO_PLACE_FROM_STACK);
            }

            if(((status.get() == STANGA_5H_PLUS.STROBOT.PLACE_PRELOAD) || (status.get() == STANGA_5H_PLUS.STROBOT.GO_PLACE_FROM_STACK)) && !drive.isBusy() && trajCount == 3) {
                trajCount++;
                drive.followTrajectorySequenceAsync(GTS_SECOND);
                status.set(STANGA_5H_PLUS.STROBOT.COLLECT);
            }

            if (status.get() == STANGA_5H_PLUS.STROBOT.COLLECT && !drive.isBusy() && trajCount == 4)
            {
                trajCount++;
                drive.followTrajectorySequenceAsync(PLACE_FIRST);
                status.set(STANGA_5H_PLUS.STROBOT.GO_PLACE_FROM_STACK);
            ...
            if(((status.get() == STANGA_5H_PLUS.STROBOT.PLACE_PRELOAD) || (status.get() == STANGA_5H_PLUS.STROBOT.GO_PLACE_FROM_STACK)) && !drive.isBusy() && trajCount == 5) {
                trajCount++;
                drive.followTrajectorySequenceAsync(GTS_THIRD);
                status.set(STANGA_5H_PLUS.STROBOT.COLLECT);
            }

            if (status.get() == STANGA_5H_PLUS.STROBOT.COLLECT && !drive.isBusy() && trajCount == 6)
            {
                trajCount++;
                drive.followTrajectorySequenceAsync(PLACE_FIRST);
                status.set(STANGA_5H_PLUS.STROBOT.GO_PLACE_FROM_STACK);
            ...
            if(((status.get() == STANGA_5H_PLUS.STROBOT.PLACE_PRELOAD) || (status.get() == STANGA_5H_PLUS.STROBOT.GO_PLACE_FROM_STACK)) && !drive.isBusy() && trajCount == 7) {
                trajCount++;
                drive.followTrajectorySequenceAsync(GTS_FOURTH);
                status.set(STANGA_5H_PLUS.STROBOT.COLLECT);
            }

            if (status.get() == STANGA_5H_PLUS.STROBOT.COLLECT && !drive.isBusy() && trajCount == 8)
            {
                trajCount++;
                drive.followTrajectorySequenceAsync(PLACE_FIRST);
                status.set(STANGA_5H_PLUS.STROBOT.GO_PLACE_FROM_STACK);
            ...
            if(((status.get() == STANGA_5H_PLUS.STROBOT.PLACE_PRELOAD) || (status.get() == STANGA_5H_PLUS.STROBOT.GO_PLACE_FROM_STACK)) && !drive.isBusy() && trajCount == 9) {
                trajCount++;
                drive.followTrajectorySequenceAsync(GTS_FIFTH);
                status.set(STANGA_5H_PLUS.STROBOT.COLLECT);
            }

            if (status.get() == STANGA_5H_PLUS.STROBOT.COLLECT && !drive.isBusy() && trajCount == 10)
            {
                trajCount++;
                drive.followTrajectorySequenceAsync(PLACE_FIRST);
                status.set(STANGA_5H_PLUS.STROBOT.PARK);
            }

            if(status.get() == STANGA_5H_PLUS.STROBOT.PARK && !drive.isBusy() && trajCount == 11) {
                trajCount++;
                if(tagOfInterest == null)
                {
                    CAZ = 2;
                ...
                status.set(STANGA_5H_PLUS.STROBOT.STOP_JOC);
            }
